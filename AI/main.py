...
while True:
    print("-------------------------------------------------------")
    prefix = f"[{state.CURRENT_FILE_NAME}] " if state.CURRENT_FILE_NAME else ""
    ...
    if state.CURRENT_MODEL is not None:
        ...
        print("state.CURRENT_MODEL-main.py",state.CURRENT_MODEL)
        ...
        if similarity < 0.6:
            ...
            if predicted_label == "handle_file":
                ...
                if action_similarity >= 0.6:
                    data_handler.route_task(predicted_action, user_text_after_handle_file)
                else:
                    print(
